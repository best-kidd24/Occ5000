...
if [ "${EXP}" = "Occ5000" ]; then
    NUM_LABELS=13
    DATA_ROOT=${ROOT_DIR}/data/Occ5000
else
    NUM_LABELS=0
    echo "Wrong exp name"
fi
...
########### voc12 ################
NET_ID=deeplabv2-VGG16


TRAIN_SET_SUFFIX=_unOcc500
...
#RUN_TRAIN=1 #train_aug
RUN_TRAIN=1
RUN_TEST=1
RUN_TRAIN2=0 #trainval_aug
RUN_TEST2=0

## Training #1 (on train_aug)
## finetune on train --kidd
...
#MODEL=${EXP}/model/${NET_ID}/train2_iter_8000.caffemodel
...
if [ ${RUN_TEST} -eq 1 ]; then
    #
    for TEST_SET in val_unOcc500; do
