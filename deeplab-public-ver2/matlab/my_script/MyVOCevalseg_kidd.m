...
confcounts = zeros(num); %confusion matrix
count=0;
...
for i=1:length(gtids) 
    ...
    try 
        ...
        gtfile = sprintf(VOCopts.seg.clsimgpath2,imname);
    end
    ...
    %pixel locations to include in computation, ignore boundary
    locs = gtim<255;
    
    % joint histogram
    sumim = 1+gtim+resim*num; %same as confusion matrix, transform to column
    hs = histc(sumim(locs),1:num*num); 
    count = count + numel(find(locs)); %gt pixels except boundaries
...
%conf = zeros(num);
conf = 100*confcounts./repmat(1E-20+sum(confcounts,2),[1 size(confcounts,2)]); %accuracy = prediction/true labels
rawcounts = confcounts;

% Pixel Accuracy
overall_acc = 100*sum(diag(confcounts)) / sum(confcounts(:));% overallpixel_acc = true postive / all prediction
