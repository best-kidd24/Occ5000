...
if is_server
  if strcmp(dataset, 'voc12')
    VOC_root_folder = '/mnt/data1/kidd/deeplab-v2/data/pascal/VOCdevkit';
...
output_mat_folder = fullfile('/mnt/data1/kidd/deeplab-v2/exper', dataset, feature_name, model_name, testset, feature_type);
%feature mat dir
save_root_folder = fullfile('/mnt/data1/kidd/deeplab-v2/exper', dataset, 'res', feature_name, model_name, testset, feature_type, post_folder);

fprintf(1, 'Saving to %s\n', save_root_folder);

if strcmp(dataset, 'voc12')
  seg_res_dir = [save_root_folder '/results/VOC2012/'];
  seg_root = fullfile(VOC_root_folder, 'VOC2012', 'VOCtrainval');
  gt_dir   = fullfile(VOC_root_folder, 'VOC2012','VOCtrainval', 'SegmentationClassAug');
elseif strcmp(dataset, 'coco')
...
if is_mat
  ...
  for i = 1 : numel(output_dir)
    ...
    if strcmp(dataset, 'voc12')
      img = imread(fullfile(VOC_root_folder, 'VOC2012', 'VOCtrainval', 'JPEGImages', [img_fn, '.jpg']));
    elseif strcmp(dataset, 'coco')
